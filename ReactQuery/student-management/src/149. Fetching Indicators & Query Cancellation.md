...
- Trong ví dụ này, chúng ta sử dụng truy vấn `useQuery` để lấy danh sách tin nhắn từ API và mutation `useMutation` để gửi tin nhắn mới. Khi người dùng gửi tin nhắn, chúng ta sử dụng `mutateAsync` để gọi API gửi tin nhắn và sau đó cập nhật danh sách tin nhắn bằng cách sử dụng `invalidateQueries`. Điều này đảm bảo rằng danh sách tin nhắn sẽ được đồng bộ hóa với dữ liệu mới nhất.
